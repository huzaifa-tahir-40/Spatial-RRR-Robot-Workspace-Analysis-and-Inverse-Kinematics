#include <math.h>
void ik_3dof(double Px, double Py, double Pz, double a1, double a2, double a3, double q[3][2])
{
/*
...
*/
  // Calculate the Angle for Joint 1 (Theta 1) 
  ...
  double k2 = Py / sin(q1);
  double k3 = Pz - a1;
  double k;              // it will be used in place of k1 or k2. (k1 = k2)

  // This if-else condition avoids division by a tiny number. Because otherwise k1 and k2 will not be equal
  if(cos(q1) < 0.01)
    k = k2;
  else if(sin(q1) < 0.01)
    k = k1;

  double cos_q3 = ((k1*k1) + (k3*k3) - (a2*a2) - (a3*a3)) / (2 * a2 * a3);
  double sin_q3 = sqrt(1 - (cos_q3*cos_q3));

  //Joint Angles for Elbow Down and Elbow Up Configuration.
  double q3_d = atan2( sin_q3 , cos_q3 );  // Theta 3 for Elbow Down
  double q3_u = atan2(-sin_q3, cos_q3 );   // Theta 3 for Elbow Up

  double q2_d = atan2(k3, r) - atan2(a3*sin(q3_d), a2 + a3*cos(q3_d)); // Theta 2 for Elbow Down
  double q2_u = atan2(k3, r) - atan2(a3*sin(q3_u), a2 + a3*cos(q3_u)); // Thata 2 for Elbow Up
  ...
  Serial.print("q2_up = "); Serial.println(round(rad2deg(q2_u)));
  Serial.print("q3_up = "); Serial.println(round(rad2deg(q3_u)));
