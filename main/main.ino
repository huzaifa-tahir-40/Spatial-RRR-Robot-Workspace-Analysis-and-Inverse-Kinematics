...
void loop()
{
  ...
  double q1   = round(rad2deg(q[0][0]));     // Theta 1 for Elbow up and Elbow down position
  double q2_u = round(rad2deg(q[1][0]));     // Theta 2 for Elbow up Joint
  double q3_u = round(rad2deg(q[2][0]));     // Theta 3 for Elbow up Joint
  double q2_d = round(rad2deg(q[1][1]));     // Theta 2 for Elbow Down Joint
  double q3_d = round(rad2deg(q[2][1]));     // Theta 3 for Elbow Down Joint
  ...
  q3_u = map(q3_u, -90, 90, 180, 0);
  q3_d = map(q3_d, -90, 90, 180, 0);
